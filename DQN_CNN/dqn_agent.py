...
class DQNAgent:
    def __init__(
        ...
        self.conv_layers_params = conv_layers if conv_layers is not None else [
            (64, (4, 4), (1, 1)), (128, (3, 3), (1, 1)), (128, (3, 3), (1, 1))
        ]
        self.pooling_layers_config = pooling_layers if pooling_layers is not None else [True, True, True]
        self.dense_neurons_list = dense_neurons if dense_neurons is not None else [512, 256]
        self.dense_activations_list = dense_activations if dense_activations is not None else ['relu', 'relu', 'linear']

        if len(self.dense_activations_list) != len(self.dense_neurons_list) + 1:
            raise ValueError("dense_activations (len {}) and dense_neurons (len {}) do not match. "
                             "Expected len(dense_activations) == len(dense_neurons) + 1.".format(
                len(self.dense_activations_list), len(self.dense_neurons_list)
            ))
    ...
    def _load_model_from_file(self, model_file_path: str) -> None:
        try:
            ...
            if loaded_model.input_shape[1:] != self.input_shape:
                print(f"Warning: Loaded model input shape {loaded_model.input_shape[1:]} "
    ...
    def add_to_memory(self, current_state: np.ndarray, next_state: np.ndarray, reward: float, done: bool) -> None:
